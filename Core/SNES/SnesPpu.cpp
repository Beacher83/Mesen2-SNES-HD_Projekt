...
bool SnesPpu::ProcessEndOfScanline(uint16_t& hClock)
{
	if(hClock >= 1364 || (hClock == 1360 && _scanline == 240 && _oddFrame && !_state.ScreenInterlace)) {
		...
		if(_scanline < _vblankStartScanline) {
			// S38: the only per-SCANLINE clock reads in the build -- skipped entirely
			// unless the measurement is switched on.
			const bool perfOn = SnesHdPerf::Enabled();
			const SnesHdPerf::Clock::time_point perfScan0 =
				perfOn ? SnesHdPerf::Clock::now() : SnesHdPerf::Clock::time_point{};
			RenderScanline();
			if(perfOn) {
		...
		if(_scanline == _nmiScanline) {
			...
			if(s_perfHaveLast && SnesHdPerf::Enabled()) {
...
void SnesPpu::RenderSprites(const uint8_t priority[4])
{
	...
	// The fringe slot follows the MAIN screen only. S25 briefly recorded it on the
	// sub screen too, for the overlay levels where DKC2 takes OBJ off main by HDMA
	// ($212C=$04) and brings the characters back through colour math; the filter
	// then mixed that fringe into the colour-math operand. It worked and drew
	// 400-750 subpixels per frame, and it was removed anyway on 08 Sep because five
	// A/B runs could not show it doing anything visible: with S25 switched off the
	// user saw no difference in Mainbrace or Lockjaw, while switching off the SUB
	// GROUND (S26/S27, below in the filter) made them report "not smoothed".
	// The likely reason it stays invisible is that the operand is ADDed to the fog
	// and often halved, which a low-alpha fringe does not survive -- untested.
	// Full implementation in commit 4482908a if it is ever worth another look.
	for(int x = _drawStartX; x <= _drawEndX; x++) {
...
void SnesPpu::SendFrame()
{
	...
	const bool perfOn = SnesHdPerf::Enabled();
	const SnesHdPerf::Clock::time_point perfUpd0 =
		perfOn ? SnesHdPerf::Clock::now() : SnesHdPerf::Clock::time_point{};
	_emu->GetVideoDecoder()->UpdateFrame(frame, isRewinding, isRewinding);
	const SnesHdPerf::Clock::time_point perfUpd1 =
		perfOn ? SnesHdPerf::Clock::now() : SnesHdPerf::Clock::time_point{};
	if(perfOn) s_perfWaitMs = SnesHdPerf::Ms(perfUpd0, perfUpd1);
	...
	if(_hdData) {
		...
		if(perfOn) s_perfClearMs = SnesHdPerf::Ms(perfUpd1, SnesHdPerf::Clock::now());
	}
