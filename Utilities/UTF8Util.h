...
namespace utf8 {
	...
	{
	public:
		ofstream(const std::string& _Str, ios_base::openmode _Mode = ios_base::out) : std::ofstream(utf8::decode(_Str), _Mode) { }
		ofstream() : std::ofstream() { }
		void open(const std::string& _Str, ios_base::openmode _Mode = ios_base::out) { std::ofstream::open(utf8::decode(_Str), _Mode); }
	};
#else
	using std::ifstream;
	using std::ofstream;
